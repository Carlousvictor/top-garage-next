...
// Resolve powershell.exe pelo caminho absoluto. Spawn por nome ("powershell.exe")
// falha com ENOENT quando o servidor Node é iniciado de um shell cujo PATH não
// inclui System32\WindowsPowerShell\v1.0 (ex.: Git Bash). Usa SystemRoot e cai
// de volta pro nome simples só se o arquivo não existir.
...
const PS_EXE = resolvePowerShell()
...
export async function printOps(ops, cfg = PRINTER_CFG) {
    ...
    try {
        ...
        const { stdout } = await execFileP(PS_EXE, args, { windowsHide: true, timeout: 15000 })
        ...
        if (e?.code === 'ENOENT' || s.includes('ENOENT')) {
            throw new ThermalError('PowerShell não encontrado no servidor. A impressão térmica só funciona com o servidor rodando localmente no Windows.', 'NO_POWERSHELL')
        }
