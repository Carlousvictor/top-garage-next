...
export const getAuthContext = cache(async () => {
    try {
        ...
        if (!user) {
            return { user: null, tenantId: null, tenant: null, role: null, actingTenantId: null }
        }

        // 1. Profile (tenant_id + role + acting_tenant_id) — tenta cache, depois Supabase.
        // Escopo efetivo de tenant:
        //   - super_admin: acting_tenant_id (NULL = neutro, não vê dados de tenant nenhum).
        //   - demais: tenant_id (home), como sempre foi.
        // Espelha exatamente o que o RLS user_tenant_id() resolve DB-side.
        let tenantId = null
        let role = null
        let actingTenantId = null
        const cachedProfile = readFromCache(profileCache, user.id)
        if (cachedProfile) {
            tenantId = cachedProfile.tenantId
            role = cachedProfile.role
            actingTenantId = cachedProfile.actingTenantId ?? null
        } else {
            // Leitura tolerante: se acting_tenant_id ainda não existe (migration
            // não rodada), faz fallback sem ela — não derruba o app.
            let prof = null
            const r1 = await supabase
                .from('profiles')
                .select('tenant_id, role, acting_tenant_id')
                .eq('user_id', user.id)
                .maybeSingle()
            ...
            const homeTenantId = prof?.tenant_id ?? null
            role = prof?.role ?? null
            actingTenantId = prof?.acting_tenant_id ?? null
            tenantId = role === 'super_admin' ? actingTenantId : homeTenantId
            writeToCache(profileCache, user.id, { tenantId, role, actingTenantId }, PROFILE_TTL_MS)
        }
