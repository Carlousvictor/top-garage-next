...
export async function GET() {
    ...
    const { data, error } = await supabase.from('brands').select('*').eq('tenant_id', tenantId).order('name')
...
export async function POST(request) {
    ...
    const { data: all } = await supabase.from('brands').select('*').eq('tenant_id', tenantId).order('name')
    return NextResponse.json({ brand: data, brands: all || [] })
}
